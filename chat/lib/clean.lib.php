...
//		$when = date('r', $usertime + C_TMZ_OFFSET*60*60);
		...
		if(C_EN_STATS)
		{
			...
			$DbLink->query("UPDATE ".C_STS_TBL." SET seconds_in=seconds_in+($usertime-last_in), longest_in=IF($usertime-last_in < longest_in, longest_in, $usertime-last_in), last_in='' WHERE stat_date=FROM_UNIXTIME(last_in,'%Y-%m-%d') AND room='$userroom' AND username='$userclosed' AND last_in!='0'");
		}
